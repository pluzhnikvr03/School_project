...
def generate_pdf_thread(chat_id, msg_id, filename, class_range="all"):
    ...
    try:
        ...
        def update_progress(status, *args):
            try:
                if status == "progress":
                    percent = args[0]
                    bot.edit_message_text(f"Создаю PDF... {percent}%", chat_id, msg_id)


                elif status == "complete":
                    bot.edit_message_text("Загружаю PDF на Google Диск...", chat_id, msg_id)

                    # Получаем токен
                    with open('token.pickle', 'rb') as token:
                        creds = pickle.load(token)

                    link = upload_to_google_drive(filename, creds)
                    if link:
                        bot.edit_message_text(f" PDF загружен!\nСсылка: {link}", chat_id, msg_id)
                    else:
                        bot.edit_message_text("Ошибка загрузки", chat_id, msg_id)

                    os.remove(filename)
            except Exception as e:
                print(f"Ошибка обновления: {e}")

        # Вызываем функцию создания PDF с callback
        excel_importer.create_qr_pdf(filename, update_progress, class_range)
...
def upload_to_google_drive(file_path, creds):
    """Загружает файл на Google Диск и возвращает ссылку для скачивания"""
    service = build('drive', 'v3', credentials=creds)

    file_metadata = {
        'name': os.path.basename(file_path),
        'parents': ['root']  # можно указать папку
    }
...
def handle_get_pdf(message):
    ...
    if message.from_user.id != config.ADMIN_ID:  # проверяем, что команду отправил администратор
        bot.reply_to(message, "Эта функция только для администратора!")
        return
    ...
    keyboard.add(
        types.InlineKeyboardButton("1–4 классы", callback_data="pdf_1_4"),
        types.InlineKeyboardButton("5–9 классы", callback_data="pdf_5_9"),
        types.InlineKeyboardButton("10–11 классы", callback_data="pdf_10_11"),
        types.InlineKeyboardButton("Все классы", callback_data="pdf_all")
    )
    bot.send_message(message.chat.id, "Выбери диапазон классов:", reply_markup=keyboard)
...
Отнеси ее учителю.
    ...
    # ========== ВЫБОР PDF С QR-КОДАМИ ==========
    if callback_data.startswith("pdf_"):
        range_type = callback_data.replace("pdf_", "")
        ...
        msg = bot.send_message(call.message.chat.id, "Создаю PDF... 0%")

        # создаем имя файла
        # datetime.now().strftime('%Y%m%d') — текущая дата в формате ГГГГММДД
        pdf_filename = f"qrcodes_{datetime.now().strftime('%Y%m%d')}_{range_type}.pdf"

        thread = threading.Thread(
            target=generate_pdf_thread,
            args=(call.message.chat.id, msg.message_id, pdf_filename, range_type)
        )
        thread.daemon = True
        thread.start()
        return
...
def handle_document(message):
    ...
    if message.from_user.id != config.ADMIN_ID:  # проверяем, что файл отправил администратор
        return

    print(f"Получен файл от пользователя {message.from_user.id}")  # пишем в консоль о получении файла
    print(f"config.ADMIN_ID = {config.ADMIN_ID}")  # пишем в консоль какой Telegram ID админа (для самопроверки)

    if message.from_user.id != config.ADMIN_ID:  # пишем в консоль о результате
