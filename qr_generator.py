...
def generate_qr_for_book(book_info, order_counter, folder="qrcodes"):  # book_info - информация о книге в любом формате; folder - папка для сохранения QR-кода; order_counter - порядковый номер qr
    ...
    # Увеличиваем разрешение QR-кода
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECT_H,
        box_size=12,  # увеличили с 8-10 до 12 (чем больше, тем чётче)
        border=2  # небольшой отступ
    )
    qr.add_data(link)
    qr.make(fit=True)

    # Создаём изображение с высоким качеством
    img = qr.make_image(fill_color="black", back_color="white").convert('RGB')

    # Увеличиваем размер для лучшей печати (опционально)
    ...
    filename = f"{order_counter:06d}_{qr_data}.png"  # создаём имя файла типа 000002_TEST-1-001.png
