...
def translator(text):
    """
    Переводит русские буквы в английские (для адекного восприятия QR)
    """
    translation = {
    ...
    result = []
    for s in text.upper():
        result.append(translation.get(s, s))
    return ''.join(result)
    ...
    try:  # пробуем обработать файл и поработать с ним
        ...
        for key, book in books_dict.items():  # key — ключ (строка), book — это словарь с данными
            ...
            for copy_num in range(1, copies + 1):
                ...
                # КОД ЧАСТИ (если есть)
                if book.get('part'):
                    part_code = translator(f"P{book['part']}")
                else:
                    part_code = 'P0'
                ...
                #           LITE-8-KOR-P2-2021-001 (часть 2)
